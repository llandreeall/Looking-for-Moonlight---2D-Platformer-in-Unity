...
public class EnemyNotAI : MonoBehaviour
{
	...
	void Flip(float _vx)
	{
		...
		Vector3 dontFlipScale = _transform.Find("StatusIndicator").transform.localScale;
		...
		_transform.Find("StatusIndicator").transform.localScale = dontFlipScale;
	}
	...
	public void Stunned()
	{
		if (!isStunned)
		{
			...
			_transform.Find("StatusIndicator").gameObject.SetActive(false);
	...
	IEnumerator Stand()
	{
		...
		_transform.Find("StatusIndicator").gameObject.SetActive(true);
	}
}
